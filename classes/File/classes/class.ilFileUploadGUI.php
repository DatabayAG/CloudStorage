...
class ilFileUploadGUI
{
    ...
    public static function initFileUpload()
    {
        ...
        // needed scripts
        $DIC->ui()->mainTemplate()->addJavaScript(ilObjCloudStorage::PLUGIN_PATH. "/classes/File/js/tmpl.js");
        $DIC->ui()->mainTemplate()->addJavaScript(ilObjCloudStorage::PLUGIN_PATH. "/classes/File/js/jquery.ui.widget.js");
        $DIC->ui()->mainTemplate()->addJavaScript(ilObjCloudStorage::PLUGIN_PATH. "/classes/File/js/jquery.iframe-transport.js");
        $DIC->ui()->mainTemplate()->addJavaScript(ilObjCloudStorage::PLUGIN_PATH. "/classes/File/js/jquery.fileupload.js");
        $DIC->ui()->mainTemplate()->addJavaScript(ilObjCloudStorage::PLUGIN_PATH. "/classes/File/js/jquery.ba-dotimeout.min.js");
        $DIC->ui()->mainTemplate()->addJavaScript(ilObjCloudStorage::PLUGIN_PATH. "/classes/File/js/ilFileUpload.js", true, 3);
        // needed styles
        $DIC->ui()->mainTmplate()->addCss(ilObjCloudStorage::PLUGIN_PATH. "/classes/File/templates/default/fileupload.css");
    }
    ...
    public function getHTML()
    {
        ...
        $onLoadCode = "";
        if ($this->use_form) {
            $onLoadCode = "var fileUpload$id = new ilFileUpload($id, " . json_encode($options) . ");";
        } else {
            $onLoadCode = "il.FileUpload.add(\"$id\", " . json_encode($options) . ", " . ($this->current_obj ? "true" : "false") . ");";
        }
