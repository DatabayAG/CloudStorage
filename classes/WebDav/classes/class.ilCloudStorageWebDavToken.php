...
/**
 * Class ilCloudStorageWebDavToken
 *
...
class ilCloudStorageWebDavToken
{
    ...
    const DB_TABLE_NAME = 'rep_robj_xcls_dav_tk';
    ...
    private function store(): void {
        global $DIC;
        $query = $DIC->database()->query("SELECT user_id FROM rep_robj_xcls_dav_tk WHERE conn_id = " . $this->getConnId() . " AND user_id = " . $this->getUserId());
        $ret = $DIC->database()->fetchAssoc($query);
        if (!is_null($ret)) {
            $DIC->database()->manipulateF(
                'UPDATE rep_robj_xcls_dav_tk SET access_token = %s, refresh_token = %s, valid_through = %s WHERE conn_id = %s AND user_id = %s',
            ...
            $DIC->database()->manipulateF(
                'INSERT INTO rep_robj_xcls_dav_tk (conn_id, user_id, access_token, refresh_token, valid_through) VALUES (%s, %s, %s, %s, %s)',
    ...
    public static function getUserToken(int $conn_id, int $user_id = 0): ilCloudStorageWebDavToken
    {
        global $DIC;
        if ($user_id == 0) {
            global $ilUser;
        ...
        $query = $DIC->database()->query("SELECT * FROM rep_robj_xcls_dav_tk WHERE conn_id = " . $conn_id . " AND user_id = " . $user_id);
        $ret = $DIC->database()->fetchAssoc($query);
        if (is_null($ret)) {
            $token = new self();
    ...
    public static function deleteUserToken(int $conn_id, int $user_id = 0): void
    {
        ...
        $DIC->database()->manipulate("DELETE FROM rep_robj_xcls_dav_tk WHERE conn_id = " . $conn_id . " AND user_id = " . $user_id);
    }
