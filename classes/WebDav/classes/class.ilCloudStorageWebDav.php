...
class ilCloudStorageWebDav implements ilCloudStorageServiceInterface
{
    ...
    public const CALLBACK_URL = 'dav_callback_url';
    
    public const AUTH_BEARER = 'dav_access_token';

    public const OAUTH2_PROVIDER_OPTIONS = 'dav_oauth2_provider_options';

    public const OAUTH2_TOKEN_REQUEST_AUTH = 'dav_oauth2_token_request_auth';
    ...
    public ?ilCloudStorageWebDavToken $user_token = null;

    public ?ilCloudStorageWebDavClient $dav_client = null;
    ...
    public function __construct(int $a_ref_id, int $connId, int $a_parent_ref_id = 0)
    {
        global $DIC;
        $this->dic = $DIC;
        $this->dic->logger()->root()->debug("ilCloudStorageWebDav __construct");
        ...
        $this->dav_client = new ilCloudStorageWebDavClient($this);
    }
    ...
    public function OAuth2Authenticate(string $callback_url): void 
    {
        ...
        if ($this->getToken()->getAccessToken() && $this->dav_client->hasConnection()) {
    ...
    public function OAuth2AfterAuthentication(): void
    {
        ...
        if (!$this->dav_client->hasConnection()) {
    ...
    public function loadToken(): void
    {
        ...
        //$this->user_token = ilCloudStorageWebDavToken::getUserToken($ilWebDav ? $ilWebDav->object->getOwnerId() : $ilUser->getId());
        assert($this->object instanceof ilObjCloudStorage);
        $this->user_token = ilCloudStorageWebDavToken::getUserToken($this->object->getConnId(), $this->object->getOwnerId());
    }
    ...
    public function checkAndRefreshAuthentication(): bool
    {
        ...
        if ($this->getToken()->isExpired()) {
            ...
            $atom_query->addTableLock(ilCloudStorageWebDavToken::DB_TABLE_NAME);
    ...
    {
        return ilObjCloudStorage::getHttpPath() . 'Customizing/global/plugins/Services/Repository/RepositoryObject/CloudStorage/classes/WebDav/redirect.php';
    }
    ...
    public function getClient(): ilCloudStorageWebDavClient
    {
