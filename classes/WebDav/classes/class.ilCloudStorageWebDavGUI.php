 ...
 *
 * @ilCtrl_isCalledBy ilCloudStorageWebDavGUI: ilObjCloudStorageGUI
 * @ilCtrl_Calls ilCloudStorageWebDavGUI: ilObjCloudStorageGUI
 *
 */

use ILIAS\DI\Container;

class ilCloudStorageWebDavGUI implements ilCloudStorageServiceGUIInterface
{
    ...
    private ?ilCloudStorageWebDav $service = null;
    ...
    public function initPropertiesForm(): void
    {
        ...
        /*
        if ($this->getAdminConfigObject()->getValue(davConfig::F_COLLABORATION_APP_INTEGRATION)) {
            $open_in_davoud = new ilCheckboxInputGUI($this->txt('allow_open_in_davoud'), 'allow_open_in_davoud');
            $open_in_davoud->setInfo($this->txt('allow_open_in_davoud_info'));
            $this->form->addItem($open_in_davoud);
        }
        */
    ...
    public function showTreeView(): void
    {
        ...
        if (!$isAsync) {
            ...
            if ($client->hasConnection()) {
                $tree = new ilCloudStorageWebDavTree($client);
                $tree_gui = new ilCloudStorageWebDavTreeGUI('tree_expl', $this->parent, 'editProperties', $tree);
    ...
    public function addItemsAfter(ilCloudStorageFileNode $node, ilAdvancedSelectionListGUI &$selection_list): void
    {
        if ($this->checkHasAction($node)) {
            ...
            $selection_list->addItem(
                $this->object->txt('open_in_davoud'),
                '',
